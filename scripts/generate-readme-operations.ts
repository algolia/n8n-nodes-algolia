...
const getOperationsForResource = (properties: INodeProperties[], resource: string) =>
    properties.filter(p => p.name === 'operation' && p.displayOptions && p.displayOptions.show?.resource?.includes(resource)).map(p => p.options).flat() as INodePropertyOptions[]


const generateContent = () => {
    const resources = properties.filter(p => p.name === 'resource')
    ...
    return resourceOperations.map(r => {
